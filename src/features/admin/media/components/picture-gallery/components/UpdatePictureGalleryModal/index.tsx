...
import useUpdatePictureGalleryModal
    from "@/features/admin/media/components/picture-gallery/hook/useUpdatePictureGalleryForm";
...
export default function Index({ reloadTable, onClose, selectedRow }: { reloadTable: () => void, onClose: () => void, selectedRow: any }) {
    const { control, errors, onSubmit, handleSubmit, updatePending, } = useUpdatePictureGalleryModal({ reloadTable: reloadTable, onClose: onClose, selectedRow: selectedRow })
    return <form className="grid grid-cols-1 gap-4 xl:grid-cols-2"
        ...
        >
            <Controller
                ...
                control={control}
                render={({ field }) => (
                    <CustomInputFile accept='image/*' value={field.value} onChange={field.onChange} multiple={true} />
                )}
            />
        </FormField>
