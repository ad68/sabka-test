...
import useUpdateFilmGalleryModal from "@/features/admin/media/components/film-gallery/hook/useUpdateFilmGalleryForm";
...
export default function Index({ reloadTable, onClose, selectedRow }: { reloadTable: () => void, onClose: () => void, selectedRow: any }) {
    const { control, errors, onSubmit, handleSubmit, updatePending, } = useUpdateFilmGalleryModal({ reloadTable: reloadTable, onClose: onClose, selectedRow: selectedRow })
    return <form className="grid grid-cols-1 gap-4 xl:grid-cols-2"
        ...
        >
            ...
            <Controller
                ...
                control={control}
                render={({ field }) => (
                    <CustomInputFile accept="video/*" value={field.value} onChange={field.onChange} />
                )}
            />
        </FormField>
