...
import useCreateSliderItemForm from "../../hooks/useCreateSliderItemForm";



export default function Index({ reloadTable, onClose }: { reloadTable: () => void, onClose: () => void }) {
    const { control, errors, handleSubmit, onSubmit, isPending } = useCreateSliderItemForm({ reloadTable: reloadTable, onClose: onClose });
    ...
    return (
        <form
            ...
            <FormField
                isError={errors.sliderFileUrl}
                errorMessage={errors?.sliderFileUrl?.message}
                title="عکس اسلایدر"
