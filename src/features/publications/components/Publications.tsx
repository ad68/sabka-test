...
export default function Index() {
  const { data, isFetching, currentPage, setCurrentPage, total } = usePublication()
  return <section className="w-[1140px] mb-20 h-auto relative  m-auto px-[20px] xl:px-0 rounded-xl mt-10 min-h-10 max-w-full">
    <h1 className="text-primary text-[32px] font-bold">نشریه ها</h1>
    <ConfigProvider
      theme={{
        token: {
          colorPrimary: "#2ebf70",
      ...
      {!isFetching &&
        <>
          <section className="grid grid-cols-1 gap-3 xl:grid-cols-3 mt-[26px]">
            {data?.elements.map((item: any, index: number) => (<Item key={index} date={gregorianToJalali(item.createdOn)} href={`/publications/${item.id}/${item.persianTitle1}`} imageSrc={item.fileUrl} title={item.persianTitle1} />
            ))}
          </section>
        </>
      }
