...
import usePublication from "@/features/landing/hooks/usePublication"
import Item from './components/Item'
import CustomButton from "@/components/kit/CustomButton"
import Link from "next/link"
export default function Index() {
    const { data, isFetching } = usePublication()
        ...
        {!isFetching &&
            <>
                <div className="w-full mx-auto grid grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4">
                    {data?.elements.slice(0, 6).map((item, index) => (<Item key={index} title={item.persianTitle1} id={item.id} date={item.createdOn} imgUrl={item.fileUrl} />))}
                </div>
